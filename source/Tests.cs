  ...
  public static void Showcase(string input) {
   ...
   try {
    ...
    foreach(var theme in UI.Themes) {
     ...
     var w=new MainWindow(s,true); w.ShowInTaskbar=false; w.ShowActivated=false; w.Width=1200; w.Height=Math.Min(950,SystemParameters.WorkArea.Height); w.Show(); w.UpdateLayout(); w.Refresh(true); w.UpdateLayout(); Capture(w,"week-"+theme.Id); Check(Descendants<System.Windows.Shapes.Line>(w).Count()>=32,"week grid renders "+theme.Id); Check(Descendants<ScrollViewer>(w).All(v=>v.ScrollableHeight<2),"fit week shows all 24 hours "+theme.Id);
