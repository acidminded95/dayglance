 ...
 public static class Tests {
  ...
  public static void UIRun() {
   ...
   try {
    ...
    Click(w,"Compact"); Check(w.State.Compact && w.Height<=550,"compact toggle"); Capture(w,"compact-preview"); Click(w,"Expand"); Check(!w.State.Compact,"expand toggle");
    Click(w,"● Pinned on top"); Check(!w.Topmost,"pin toggle");
    Click(w,"Week"); w.UpdateLayout(); Check(w.State.WeekView && Descendants<System.Windows.Shapes.Line>(w).Count()>=32,"week toggle"); double oldHeight=Descendants<Canvas>(w).Max(c=>c.Height); w.ZoomSchedule(1,100); w.UpdateLayout(); Check(Descendants<Canvas>(w).Max(c=>c.Height)>oldHeight,"week zoom increases timeline scale"); w.ZoomSchedule(-1,100); w.UpdateLayout(); Check(Math.Abs(Descendants<Canvas>(w).Max(c=>c.Height)-oldHeight)<.1,"week zoom reverses"); Click(w,"Day"); Check(!w.State.WeekView,"day toggle");
